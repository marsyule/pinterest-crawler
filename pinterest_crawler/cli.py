...
def build_parser() -> argparse.ArgumentParser:
    ...
    download_user = subparsers.add_parser(
        "download-user",
        help="download all public boards from a Pinterest user profile",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "download-user":
        ...
        completed = sum(1 for board in user_manifest.boards if board.status == "complete")
        failed = sum(1 for board in user_manifest.boards if board.status == "failed")
        print(
            f"User {user_manifest.username}: status={user_manifest.status}, "
            f"completed={completed}/{len(user_manifest.boards)}, failed={failed}"
        )
        return 0
